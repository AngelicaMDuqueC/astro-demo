// To parse this data:
//
//   import { Convert, APISpaceXTypes } from "./file";
//
//   const aPISpaceXTypes = Convert.toAPISpaceXTypes(json);
//
// These functions will throw an error if the JSON doesn't
// match the expected interface, even if the JSON is valid.

export interface APISpaceXTypes {
    docs:          Doc[];
    totalDocs:     number;
    offset:        number;
    limit:         number;
...
export class Convert {
    public static toAPISpaceXTypes(json: string): APISpaceXTypes {
        return cast(JSON.parse(json), r("APISpaceXTypes"));
    }

    public static aPISpaceXTypesToJson(value: APISpaceXTypes): string {
        return JSON.stringify(uncast(value, r("APISpaceXTypes")), null, 2);
    }
}
